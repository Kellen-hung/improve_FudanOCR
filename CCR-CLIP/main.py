...
print(len(alphabet))
# print(alphabet)
print('\033[32mLoading model ...\033[0m')
...
print('\033[35mDone!\033[0m')
print('\033[32mPre-Processing ... (dataloader, loss)\033[0m')
...
def val(model):
    ...
    tmp_text = convert(char_5018)
    text_features = []
    iters = len(char_5018) // 100
    with torch.no_grad():
...
for epoch in range(config['epoch']):
    ...
    for iteration in range(train_loader_len):
        ...
        # # 建立 char -> index 的字典（建議放在迴圈外建一次即可）
        # char2idx = {ch: i for i, ch in enumerate(alphabet)}

        # # 把 label 轉成 ground truth index
        # ground_truth = torch.zeros(len(label), dtype=torch.long).cuda()
        # for i, ch in enumerate(label):
        #     ground_truth[i] = char2idx.get(ch, -1)  # 如果字不在 alphabet 就是 -1（可視情況忽略或處理）
